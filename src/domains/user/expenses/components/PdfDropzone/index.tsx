    ...
    fileRejections.forEach(file => {
        file.errors.forEach(err => {
            ...
            if (err.code === 'too-many-files') {
                toast.error('Too many files. Maximum number of files is 1.');
            }
        });
    });
};
...
export default function PdfDropzone({
    ...
    return (
        <div
            {...getRootProps()}
            className={`w-full h-full p-1 custom-dashed bg-light flex flex-col justify-center items-center text-default-700 cursor-pointer ${isDragActive ? 'bg-primary-50' : ''}`}
        >
            <input {...getInputProps()} />
            {renderBody}
        </div>
